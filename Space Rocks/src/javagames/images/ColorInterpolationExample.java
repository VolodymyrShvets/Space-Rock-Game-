...
public class ColorInterpolationExample extends SimpleFramework {
    ...
    public ColorInterpolationExample() {
        ...
        appHeight = 640;
        appSleep = 1L;
        appTitle = "ColorInterpolationExample";
    }
    ...
    private void createColorSquare() {
        ...
        for (int row = 0; row < h; row++) {
            ...
            int lg = (int) (tlr + (row - h0) * (blg - tlg) / h1h0);
            int lb = (int) (tlr + (row - h0) * (blb - tlb) / h1h0);

            // Right pixel
            int rr = (int) (trr + (row - h0) * (brr - trr) / h1h0);
            int rg = (int) (trr + (row - h0) * (brg - trg) / h1h0);
            int rb = (int) (trr + (row - h0) * (brb - trb) / h1h0);
