...
public class LoadXMLExample {
    ...
    private void parseXML(Document document) {
        ...
        List<Element> text =
                XMLUtility.getAllElements(element, "text");
        System.out.println("Element: " + text.get(0).getTextContent());

        List<Element> nested =
                XMLUtility.getAllElements(element, "nested");
        System.out.println("Element: " + nested.get(0).getNodeName());

        List<Element> elementChildren =
                XMLUtility.getAllElements(element, "children");
        System.out.println("Get-Child-Count: " + elementChildren.size());

        List<Element> child =
                XMLUtility.getAllElements(nested.get(0), "child");
        System.out.println("Get-Child-Count: " + child.size());

        List<Element> allChildren =
                XMLUtility.getAllElements(nested.get(0), "child");
        System.out.println("Get-Child-Count: " + allChildren.size());

        List<Element> nestedText =
                XMLUtility.getAllElements(element, "nested-text");
        System.out.println("nested-text: " + nestedText.get(0).getTextContent());

        List<Element> nestedChild =
                XMLUtility.getAllElements(nestedText.get(0), "child");
        ...
        System.out.println("special-chars: " + specialChars.get(0).getTextContent());

        List<Element> cdata =
                XMLUtility.getElements(element, "cdata");
